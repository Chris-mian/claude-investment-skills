...
|---|---|---|
| **2.4** | 2026-05-12 | **Theme classifier — dual-path detection.** `strategic-partner-firehose/scripts/classifier.py` adds a parallel detection path that fires on AI/datacenter/hyperscaler theme density (7 keyword categories scored 0-10) when the registry-based path misses. **Catches POWL-style alerts** ($400M data center order from "a major U.S. technology company" — anonymous customer, no investor name match, but theme score 10/10). Whitespace normalization in classifier makes regex robust to HTML-stripped filings. 36 unit tests pass (4 new for classifier). README rewritten with concrete POWL +73% in 7 days case study. |
| **2.3** | 2026-05-12 | **Composite MEGA SIGNAL detector** — `strategic-partner-firehose/scripts/composite.py` cross-references insider-firehose + strategic-partner-firehose. When same ticker fires both alerts within 30 days = 🚨🚨🚨 mega signal alert (rare, < 1% of all alerts). Both firehoses now log to `recent_alerts.json` (90-day retention). Cron aligned to */30 (was hourly) so both fire on same tick. README rewritten with concrete CoreWeave (CRWV) backtest: 2025-03-31 8-K Item 1.01 (OpenAI $350M PIPE) → IPO $40 → ATH $187 in 3 months. |
